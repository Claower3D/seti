...
defer wg.Done()
clientID := "56d30cce"
searchURL := fmt.Sprintf("https://api.jamendo.com/v3.0/tracks/?client_id=%s&format=json&limit=15&search=%s", clientID, url.QueryEscape(query))
resp, err := http.Get(searchURL)
if err != nil { return }
defer resp.Body.Close()
var res struct {
...
for _, t := range res.Results {
allSongs = append(allSongs, models.Song{
Title: t.Name, Artist: t.Artist, URL: "/api/music/proxy/raw?url=" + url.QueryEscape(t.Audio), ImageURL: t.Image, Source: "jamendo", Duration: t.Duration,
})
}
mu.Unlock()
}
...
req.Header.Set("User-Agent", userAgent)
if strings.Contains(finalStreamURL, "itunes.apple.com") || strings.Contains(finalStreamURL, "audio-ssl.itunes.apple.com") {
