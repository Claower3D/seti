...
export const MessagesPage = () => {
  ...
  return (
    <div style={{ height: 'calc(100vh - 140px)', display: 'flex', gap: '16px' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px' }}>
            {friends.length === 0 ? (
              <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--text-secondary)' }}>
                <MessageSquare size={48} className="neon-text-purple" style={{ marginBottom: '16px', opacity: 0.3 }} />
              ...
              <div key={friend.id} onClick={() => setSelectedFriend(friend)}
                style={{ padding: '16px', display: 'flex', gap: '14px', cursor: 'pointer', alignItems: 'center', borderRadius: '16px',
                  background: selectedFriend?.id === friend.id ? 'rgba(0, 242, 255, 0.08)' : 'transparent',
                  border: selectedFriend?.id === friend.id ? '1px solid rgba(0, 242, 255, 0.2)' : '1px solid transparent',
                  transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                  marginBottom: '8px' }}>
                <div style={{ position: 'relative' }}>
                  <img src={friend.avatar || 'https://api.dicebear.com/7.x/avataaars/svg?seed=' + friend.username}
                    alt="avatar" style={{ width: '52px', height: '52px', borderRadius: '18px', objectFit: 'cover', border: selectedFriend?.id === friend.id ? '2px solid var(--primary-color)' : '2px solid transparent' }} />
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: '800', fontSize: '1rem', color: selectedFriend?.id === friend.id ? 'var(--primary-color)' : 'white' }}>{friend.username}</div>
      ...
      {/* Chat window */}
      {showChat && (
        <div className="glass-panel" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0, border: '1px solid rgba(0, 242, 255, 0.1)' }}>
          {selectedFriend ? (
            <>
              {/* Chat header */}
              <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '14px', background: 'rgba(255,255,255,0.02)' }}>
                {isMobile && (
                  <button onClick={() => setSelectedFriend(null)}
                    style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', padding: '8px', display: 'flex', borderRadius: '12px', transition: 'all 0.2s' }}
                ...
                <img src={selectedFriend.avatar || 'https://api.dicebear.com/7.x/avataaars/svg?seed=' + selectedFriend.username}
                  alt="avatar" style={{ width: '44px', height: '44px', borderRadius: '14px', border: '2px solid var(--primary-color)' }} />
                <div>
                  <div style={{ fontWeight: '900', fontSize: '1.1rem' }} className="neon-text">{selectedFriend.username}</div>
